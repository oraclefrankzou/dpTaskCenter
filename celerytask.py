...
class ServiceAnsible():
    ...
    def exec_ansible(self, host):
        ...
        #需要手动生hosts文件夹
        filename="/tmp/hosts"+str(int((ipaddress.ip_address(host))))
...
celery = Celery("dp", broker="redis://10.0.0.3:6379/0",backend="redis://10.0.0.3:6379/0")



@celery.task
def celeryTask(module,args,params):
    result={}
    serviceAnsible = ServiceAnsible(module=module, args=args)
    result = serviceAnsible.exec_ansible(params["host"])
    return result